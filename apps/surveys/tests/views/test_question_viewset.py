...
def test_question_create_published_survey(admin_api_client, published_survey):
    data = {"text": "test text", "survey": str(published_survey.id)}
    url = reverse("surveys:question-list")
    response = admin_api_client.post(url, data, format="json")
...
def test_question_update_published_survey(admin_api_client, published_survey):
    data = {
        "text": "test text",
        "order": 2,
        "is_required": False,
        "survey": str(published_survey.id),
    }
    question = QuestionFactory.create(survey=published_survey)
    url = reverse("surveys:question-detail", kwargs={"pk": str(question.id)})
    response = admin_api_client.put(url, data=data, format="json")
...
def test_question_partial_update_published_survey(admin_api_client, published_survey):
    data = {"is_required": False}
    question = QuestionFactory.create(survey=published_survey, is_required=True)
    url = reverse("surveys:question-detail", kwargs={"pk": str(question.id)})
    response = admin_api_client.patch(url, data=data, format="json")
...
def test_question_delete_published_survey(admin_api_client, published_survey):
    question = QuestionFactory.create(survey=published_survey)
    url = reverse("surveys:question-detail", kwargs={"pk": str(question.id)})
    response = admin_api_client.delete(url)
